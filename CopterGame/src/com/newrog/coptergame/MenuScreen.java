...
public class MenuScreen implements Screen {
	...
	public MenuScreen(CopterGame g) {
		game = g;
		junk = new Junk();
		junkgaming = new Junk();
		...
		junk.position.y = Gdx.graphics.getHeight()+40;
		...
		Tween.to(junk, EntityAccessor.POSITION_XY, 1.0f).delay(0.5f).target(Gdx.graphics.getWidth()/2-junk.sprite.getWidth()/2,
				Gdx.graphics.getHeight()/2+2.1f*junk.sprite.getHeight()+1)
				.ease(Bounce.OUT).delay(1.0f).setCallback(callback).start(tm);
		Tween.to(junkgaming, EntityAccessor.POSITION_XY, 1.0f).delay(0.5f).target(Gdx.graphics.getWidth()/2-junkgaming.sprite.getWidth()/2,
				Gdx.graphics.getHeight()/2-2.1f*junkgaming.sprite.getHeight()-1)
				.ease(Bounce.OUT).delay(1.0f).start(tm);
