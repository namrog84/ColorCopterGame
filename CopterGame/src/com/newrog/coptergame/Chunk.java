...
public class Chunk {
	...
	public static final int chunkWidth = 35;
	private static final int chunkHeight = 60;
	...
	private void buildLevel(Entity[][] bb, int sX, int sY) {
		...
		for (int i = 0; i < chunkWidth - sX; i++) {
			if (r.nextInt(100) <= roughness) {
				...
				if (currentWidth < 17 - chunkDiff)
					currentWidth = 17 - chunkDiff;
			...
			//for (int j = 0; j < currentWidth; j++) {
				//if (bb[sX + i][sY + j].type == 3) {
					...
					//}
				//}
			bb[sX + i][sY + 0] = new Block(chunkOffset + sX * 32 + i * 32,    0 , game, sY*32, true);
			bb[sX + i][sY+currentWidth] = new Block(chunkOffset + sX * 32 + i * 32, (currentWidth+sY)*32, game, (32*chunkHeight), false);
			//}
			((Block) bb[sX + i][sY]).addBox();
			
			((Block) bb[sX + i][sY + currentWidth]).addBox();
...
//				((Block) bb[sX + i-1][sY + currentWidth - z] ).addBox(); // add top
	...
	void fillBlocks(Entity[][] b, int x) {
		for (int i = 0; i < chunkWidth; i++) {
			for (int j = 0; j < chunkHeight; j++) {
				b[i][j] = new EmptyBlock();//chunkOffset + x * 32 + i * 32, j * 32, game);
	...
	public void reset() {
		...
		if (chunkPosition == 0) {
			...
			blocks[0][20] = new Block(0, 0, game, chunkHeight*32, true);
			blocks[1][20] = new Block(32, 0, game, chunkHeight*32, true);
						
			for (int i = 2; i < 10; i++) {
				blocks[i][20] = new Block(i*32, 0, game, 20*32, true);
				((Block) blocks[i][20]).addBox();
				
				blocks[i][30] = new Block(i*32, 30*32, game, 60*32, false);
				((Block) blocks[i][30]).addBox();
			}
		
			
			//blocks[sX + i][sY+currentWidth] = new Block(chunkOffset + sX * 32 + i * 32, (currentWidth+sY)*32, game, (32*chunkHeight), false);
			//}
			//((Block) bb[sX + i][sY]).addBox();
